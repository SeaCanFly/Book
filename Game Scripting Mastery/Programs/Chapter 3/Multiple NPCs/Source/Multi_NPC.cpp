/*
    ...
    Abstract.
        ...
        Scripts the behavior of non-player characters in a role-playing game.
...
// ---- Constants -----------------------------------------------------------------------------
    ...
    // ---- NPCs ------------------------------------------------------------------------------
        ...
        #define MAX_NPC_COUNT               5           // Maximum number of NPCs
...
// ---- Data Structures -----------------------------------------------------------------------
    ...
    // ---- NPC -------------------------------------------------------------------------------
        ...
        typedef struct _NPC
        {
            int iIsActive;                              // Is this NPC active?
...
// ---- Global Variables ----------------------------------------------------------------------
    ...
    // ---- NPC -------------------------------------------------------------------------------
        ...
        NPC g_NPCs [ MAX_NPC_COUNT ];                   // The NPCs
...
// ---- Function Prototypes -------------------------------------------------------------------
    ...
    // ---- Scripts ---------------------------------------------------------------------------
        ...
        int LoadScript ( char * pstrFilename, int iNPCIndex );
        void UnloadScript ( int iNPCIndex );
        void ResetScript ( int iNPCIndex );
        void RunScript ( int iNPCIndex );

    // ---- Source Code -----------------------------------------------------------------------

        void GetCommand ( char * pstrDestString, int iNPCIndex );
        int GetIntParam ( int iNPCIndex );
        void GetStringParam ( char * pstrDestString, int iNPCIndex );
...
// ---- Functions -----------------------------------------------------------------------------
    ...
    */
    ...
    int LoadScript ( char * pstrFilename, int iNPCIndex )
    {
        ...
        g_NPCs [ iNPCIndex ].ppstrScript = ( char ** ) malloc ( g_NPCs [ iNPCIndex ].iScriptSize * sizeof ( char * ) );

        // Load each line of code

        for ( int iCurrLineIndex = 0; iCurrLineIndex < g_NPCs [ iNPCIndex ].iScriptSize; ++ iCurrLineIndex )
        {
            // Allocate space for the line and a null terminator

            g_NPCs [ iNPCIndex ].ppstrScript [ iCurrLineIndex ] = ( char * ) malloc ( MAX_SOURCE_LINE_SIZE + 1 );

            // Load the line

            fgets ( g_NPCs [ iNPCIndex ].ppstrScript [ iCurrLineIndex ], MAX_SOURCE_LINE_SIZE, pScriptFile );
        }

        // Activate the NPC

        g_NPCs [ iNPCIndex ].iIsActive = TRUE;
    ...
    */
    ...
    void UnloadScript ( int iNPCIndex )
    {
        ...
        if ( ! g_NPCs [ iNPCIndex ].ppstrScript )
            return;

        // Free each line of code individually

        for ( int iCurrLineIndex = 0; iCurrLineIndex < g_NPCs [ iNPCIndex ].iScriptSize; ++ iCurrLineIndex )
            free ( g_NPCs [ iNPCIndex ].ppstrScript [ iCurrLineIndex ] );

        // Free the script structure itself

        free ( g_NPCs [ iNPCIndex ].ppstrScript );
    ...
    */
    ...
    void GetCommand ( char * pstrDestString, int iNPCIndex )
    {
        ...
        while ( g_NPCs [ iNPCIndex ].iCurrScriptLineChar < ( int ) strlen ( g_NPCs [ iNPCIndex ].ppstrScript [ g_NPCs [ iNPCIndex ].iCurrScriptLine ] ) )
        {
            // Read the next character from the line

            cCurrChar = g_NPCs [ iNPCIndex ].ppstrScript [ g_NPCs [ iNPCIndex ].iCurrScriptLine ][ g_NPCs [ iNPCIndex ].iCurrScriptLineChar ];
            ...
            ++ g_NPCs [ iNPCIndex ].iCurrScriptLineChar;
        }
    ...
    */
    ...
    void GetStringParam ( char * pstrDestString, int iNPCIndex )
    {
        ...
        g_NPCs [ iNPCIndex ].iCurrScriptLineChar += 2;
    ...
    */
    ...
    void ResetScript ( int iNPCIndex )
    {
        ...
        g_NPCs [ iNPCIndex ].iCurrScriptLine = 0;
    }
    ...
    *   RunScript ()
    *
    *   Runs the next command in the specified script.
    */

    void RunScript ( int iNPCIndex )
    {
        // If the script is inactive, return

        if ( ! g_NPCs [ iNPCIndex ].iIsActive )
            return;
        ...
        if ( g_NPCs [ iNPCIndex ].iMoveX || g_NPCs [ iNPCIndex ].iMoveY )
            return;

        // Return if the script is currently paused

        if ( g_NPCs [ iNPCIndex ].iIsPaused )
            if ( W_GetTickCount () > g_NPCs [ iNPCIndex ].iPauseEndTime )
                g_NPCs [ iNPCIndex ].iIsPaused = TRUE;
            else
                return;

        // If the script is finished, loop back to the start

        if ( g_NPCs [ iNPCIndex ].iCurrScriptLine >= g_NPCs [ iNPCIndex ].iScriptSize )
            g_NPCs [ iNPCIndex ].iCurrScriptLine = 0;
        ...
        if ( strlen ( g_NPCs [ iNPCIndex ].ppstrScript [ g_NPCs [ iNPCIndex ].iCurrScriptLine ] ) == 0 ||
             ( g_NPCs [ iNPCIndex ].ppstrScript [ g_NPCs [ iNPCIndex ].iCurrScriptLine ][ 0 ] == '/' &&
               g_NPCs [ iNPCIndex ].ppstrScript [ g_NPCs [ iNPCIndex ].iCurrScriptLine ][ 1 ] == '/' ) )
        {
            // Move to the next line

            ++ g_NPCs [ iNPCIndex ].iCurrScriptLine;
        ...
        g_NPCs [ iNPCIndex ].iCurrScriptLineChar = 0;

        // Read the command

        GetCommand ( pstrCommand, iNPCIndex );
        ...
        if ( stricmp ( pstrCommand, COMMAND_MOVECHAR ) == 0 )
        {
            ...
            g_NPCs [ iNPCIndex ].iMoveX = GetIntParam ( iNPCIndex );
            g_NPCs [ iNPCIndex ].iMoveY = GetIntParam ( iNPCIndex );
        }
        ...
        if ( stricmp ( pstrCommand, COMMAND_SETCHARLOC ) == 0 )
        {
            ...
            int iX = GetIntParam ( iNPCIndex ),
                iY = GetIntParam ( iNPCIndex );

            // Calculate the distance to this location

            int iXDist = iX - g_NPCs [ iNPCIndex ].iX,
                iYDist = iY - g_NPCs [ iNPCIndex ].iY;

            // Set the player along this path

            g_NPCs [ iNPCIndex ].iMoveX = iXDist;
            g_NPCs [ iNPCIndex ].iMoveY = iYDist;
        }
        ...
        else if ( stricmp ( pstrCommand, COMMAND_SETCHARDIR ) == 0 )
        {
            ...
            GetStringParam ( pstrStringParam, iNPCIndex );

            if ( stricmp ( pstrStringParam, "Up" ) == 0 )
                g_NPCs [ iNPCIndex ].iDir  = UP;

            if ( stricmp ( pstrStringParam, "Down" ) == 0 )
                g_NPCs [ iNPCIndex ].iDir  = DOWN;

            if ( stricmp ( pstrStringParam, "Left" ) == 0 )
                g_NPCs [ iNPCIndex ].iDir  = LEFT;

            if ( stricmp ( pstrStringParam, "Right" ) == 0 )
                g_NPCs [ iNPCIndex ].iDir  = RIGHT;
        }
        ...
        else if ( stricmp ( pstrCommand, COMMAND_PAUSE ) == 0 )
        {
            ...
            int iPauseDur = GetIntParam ( iNPCIndex );
            ...
            g_NPCs [ iNPCIndex ].iIsPaused = TRUE;
            g_NPCs [ iNPCIndex ].iPauseEndTime = iPauseEndTime;
        }
...
// ---- Main ----------------------------------------------------------------------------------
    ...
    Main
    {
        ...
        {
            g_NPCs [ iCurrNPCIndex ].iIsActive = FALSE;
            g_NPCs [ iCurrNPCIndex ].iDir = DOWN;
            g_NPCs [ iCurrNPCIndex ].iX = 296;
            g_NPCs [ iCurrNPCIndex ].iY = 208;
            g_NPCs [ iCurrNPCIndex ].iMoveX = 0;
            g_NPCs [ iCurrNPCIndex ].iMoveY = 0;
            g_NPCs [ iCurrNPCIndex ].iIsPaused = FALSE;
        }

        // Load the scripts

        if ( ! LoadScript ( "npc_0.cbl", 0 ) )
            W_ExitOnError ( "Could not load NPC 0 script." );
        if ( ! LoadScript ( "npc_1.cbl", 1 ) )
            W_ExitOnError ( "Could not load NPC 1 script." );
        if ( ! LoadScript ( "npc_2.cbl", 2 ) )
            W_ExitOnError ( "Could not load NPC 2 script." );
        if ( ! LoadScript ( "npc_3.cbl", 3 ) )
            W_ExitOnError ( "Could not load NPC 3 script." );
        if ( ! LoadScript ( "npc_4.cbl", 4 ) )
            W_ExitOnError ( "Could not load NPC 4 script." );
        ...
        strcpy ( g_pstrTextBoxMssg, "MULTIPLE NPC SCRIPTS RUNNING CONCURRENTLY." );
        ...
        // Create a string for holding the NPCs' indices

        char pstrIndexString [ 24 ];
...

        {
            HandleLoop
            {
                // Execute the next command from each active script

                for ( iCurrNPCIndex = 0; iCurrNPCIndex < MAX_NPC_COUNT; ++ iCurrNPCIndex )
                    RunScript ( iCurrNPCIndex );
                ...
                for ( iCurrNPCIndex = 0; iCurrNPCIndex < MAX_NPC_COUNT; ++ iCurrNPCIndex )
                {
                    switch ( g_NPCs [ iCurrNPCIndex ].iDir )
                    {
                        case UP:
                            if ( iCurrAnimFrame )
                    ...
                    W_BlitImage ( * phCurrFrame, g_NPCs [ iCurrNPCIndex ].iX, g_NPCs [ iCurrNPCIndex ].iY );

                    // Print a blinking NPC index above it

                    if ( iCurrAnimFrame )
                    {
                        sprintf ( pstrIndexString, "%d", iCurrNPCIndex );
                        W_DrawTextString ( pstrIndexString, g_NPCs [ iCurrNPCIndex ].iX, g_NPCs [ iCurrNPCIndex ].iY );
                    }
                }
                ...
                for ( iCurrNPCIndex = 0; iCurrNPCIndex < MAX_NPC_COUNT; ++ iCurrNPCIndex )
                {
                    ...
                    if ( W_GetTimerState ( g_hMoveTimer ) )
                    {
                        ...
                        if ( g_NPCs [ iCurrNPCIndex ].iMoveX > 0 )
                        {
                            ++ g_NPCs [ iCurrNPCIndex ].iX;
                            -- g_NPCs [ iCurrNPCIndex ].iMoveX;
                        }
                        if ( g_NPCs [ iCurrNPCIndex ].iMoveX < 0 )
                        {
                            -- g_NPCs [ iCurrNPCIndex ].iX;
                        ...
                        if ( g_NPCs [ iCurrNPCIndex ].iMoveY > 0 )
                        {
                            ++ g_NPCs [ iCurrNPCIndex ].iY;
                            -- g_NPCs [ iCurrNPCIndex ].iMoveY;
                        }
                        if ( g_NPCs [ iCurrNPCIndex ].iMoveY < 0 )
                        {
                            -- g_NPCs [ iCurrNPCIndex].iY;
                ...
                if ( g_iExitApp || W_GetKeyState ( W_KEY_ESC ) )
                    break;
            }
        }
